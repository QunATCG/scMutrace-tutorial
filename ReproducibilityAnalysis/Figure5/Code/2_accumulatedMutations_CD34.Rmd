...
CD34_cell_mut_raw <- CD34_CSM[cells_candidate, muts_candidate]
...
rowname_day <- data.frame(rowname = rownames(data_check_CM), day = sub("_[^_]+$", "", rownames(data_check_CM)), stringsAsFactors = FALSE)
...
for (iter in seq_len(iteration_times)) {
  ...
  tab <- table(sub(".*_(Day[0-9]+)_.*", "\\1", rownames(data_cell_mut_raw_check)))
...
p_scMutrace_iter_GroupQunScaled <- ggplot(data = data_scMutModel_res_summary, mapping = aes(x = GroupQun, y = mean_scMutScore_scaled, fill = GroupQun)) +
 ...
 p_scMutrace_iter_Day <- ggplot(data = data_scMutModel_res_summary_Day, mapping = aes(x = day, y = mean_scMutScore, fill = day)) +
  geom_boxplot(outliers = FALSE) +
  labs(x = "", y = "scMutraceScore") +
  scale_fill_manual(values = c(CD34_800_Day08 = "#c7e9c0", CD34_800_Day14 = "#74c476", CD34_800_Day20 = "#005a32")) +
  theme_classic()
 
 p_scMutrace_iter_Day_scaled <- ggplot(data = data_scMutModel_res_summary_Day, mapping = aes(x = day, y = mean_scMutScore_scaled, fill = day)) +
  geom_boxplot(outliers = FALSE) +
  ylim(0, 1) +
  labs(x = "", y = "mean_scMutScore_scaled") +
  scale_fill_manual(values = c(CD34_800_Day08 = "#c7e9c0", CD34_800_Day14 = "#74c476", CD34_800_Day20 = "#005a32")) +
...
data_scMutModel_res_summary_mean_rmprog <- data_scMutModel_res_summary_mean[!data_scMutModel_res_summary_mean$lineage %in% c("prog"),]
